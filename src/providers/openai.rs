use crate::providers::{AiProvider, AiResponse, AiContext, ProviderConfig, ProviderError, ProviderResult, build_system_prompt, parse_tool_calls};
...
impl AiProvider for OpenAiProvider {
    async fn complete(&self, prompt: &str, context: &AiContext) -> ProviderResult<AiResponse> {
        ...
        let choices = json["choices"].as_array()
            .ok_or_else(|| ProviderError::ParseError("Response missing 'choices' array".to_string()))?;

        if choices.is_empty() {
            return Err(ProviderError::ParseError("API returned empty choices array".to_string()));
        }
