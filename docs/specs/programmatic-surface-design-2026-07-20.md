...
2. WIRE PROTOCOL (JSON-RPC over the Unix socket): newline-delimited `{id, method, params}` → `{id, ok, result|error:{code,message}}`, plus an additive optional `stream: bool` field for the streamed `run`. Canonical method names, agreed by all clients: `session.create/list/close`, `page.create/close/list`, `navigate`, `snapshot`, `tool.exec`, `list_tools` (+ `tools.list` returning `Vec<ToolDefinition>`), `ask`, `run` (streaming), `approval.resolve`, `run.cancel`, `worker.spawn/list/get`, `policy.get/set/evaluate/snapshot`, `skills.get`. Every method takes explicit `session_id`/`page_id` in params (mirrors main.rs Tauri command signatures 1:1). Shipped `docs/AGENT-SURFACE.md` is 17 CSS-selector tools (no `ref_map`). This design's planned CLI/MCP verbs still assume a future ref-based contract that is not in the crate.
...
- `docs/AGENT-SURFACE.md` — the 17 CSS-selector tools (`navigate`, `wait`, `query_dom`, `get_text`, `get_links`, `get_prices`, `get_tables`, `click`, `type`, `scroll_to`, `scroll_by`, `submit_form`, `keypress`, `screenshot`, `back`, `forward`, `reload`). Selector-based; `PageSnapshot` has no `ref_map`. Not shipped as named tools: `evaluate`, `get_attribute`, `wait_for`, `extract_text`. Headless JSON-RPC today is `ping` / `policy.*` / stub `snapshot` only.
- `SKILL.md` documents in-process crate use and the daemon's `ping` / `policy.*` / stub `snapshot` JSON-RPC. There is no `neurobrowser-cli` binary; this design is the proposed CLI, not a shipped wrapper.
...
2. **Browser tool surface** (1:1 with the 17 CSS-selector tools in AGENT-SURFACE.md — each becomes a first-class verb, matching `agent-browser`'s per-verb style like `click`/`fill`/`type`, rather than one generic passthrough):
