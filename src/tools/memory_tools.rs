...
//! `neuro_memory::MemoryService`. That store is durable page memory. It is
//! not `crate::agent::memory::AgentMemory`, which only lives for one agent run.
//!
...
impl BrowserTool for SearchPersonalMemoryTool {
    ...
    fn description(&self) -> &str {
        "Search persistent personal page memory (MemoryService). This is not the in-run agent log (AgentMemory)."
    }
...
impl BrowserTool for InspectActivePageTool {
    ...
    fn description(&self) -> &str {
        "Return captured personal-memory content for the current page URL, or a policy-denied error. Uses MemoryService, not in-run AgentMemory."
    }
